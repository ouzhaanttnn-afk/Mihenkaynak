      ...
      {session.offerHistory.length > 0 && (
        <div className="history">
          ...
          {session.offerHistory.map((offer, i) => {
            ...
            // (GDD 11.4). TAM EŞİTLİK — `negotiation.ts`'nin saydığı şeyle
            // birebir aynı olmalı, yoksa arayüz "tekrar" derken motor
            // saymaz (ya da tersi) ve oyuncu neden ceza yediğini anlayamaz.
            const isRepeat = prev !== undefined && offer === prev;
            return (
              <span
                key={`${i}-${offer}`}
