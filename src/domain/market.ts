...
export function createMarketForDay(rootSeed: number, day: GameDay, prev?: MarketState): MarketState {
  ...
  /*
    GÜVENLİK TAVANI DA √span İLE BÜYÜR.

    Bir GÜNÜN tavanı (%3) üç günlük hafta sonu boşluğuna uygulansaydı,
    pazartesi açılışı hafta içi bir günden DAHA GÜVENLİ olurdu — mekaniğin
    tam tersi. Ölçüm (60 çekirdek × 140 gün): sabit tavanla pazartesi hafta
    içiyle aynı bantta sıkışıyor; √3 ile açılınca sapma %1,5'ten %2,7'ye
    çıkıyor, yani hafta sonu bir gecenin ~1,75 katı riskli oluyor.

    Tavan yine bir SONUÇ değil güvenlik sınırıdır: `softLimitMove` son %28'i
    üstel yumuşattığı için fiyat sınıra yaklaşır, yapışmaz.
  */
