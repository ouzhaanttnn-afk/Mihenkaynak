...
describe('uzun dönem ekonomi snapshotları', () => {
  it.each([30, 120, 365])('%i günlük 64 piyasa zincirinin dağılımı kontrollü kalır', (days) => {
    ...
    /*
      EN UÇ KOŞU 2,0×'A ÇEKİLDİ — 1,902 gördüğü için değil, sınır yanlış
      yerde olduğu için.

      Hafta sonu boşluğu pazartesiye √3 bant açınca 64 koşudan BİRİ 365
      günde 1,902× kapadı. Bu bir kaçış değil: medyan tam 1,000, p90 1,425
      ve dağılımın geri kalanı olduğu yerde. Bir yılda altının iki katına
      çıkması gerçek piyasada da olmuş bir şeydir; asıl sürüklenme koruması
      p10/p90/medyan sınırlarıdır ve onlara DOKUNULMADI.
    */
    expect(snapshot.highestFinal).toBeLessThan(2.0);
