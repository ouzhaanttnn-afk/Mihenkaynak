...
import { closedDaysBefore, isMarketOpen, isShopOpen, weekdayLabel } from './calendar';
...
describe('takvim ve piyasa kapanışı', () => {
  ...
  /*
    HAFTA İÇİ ±%3; PAZARTESİ ±%3 × √3 ≈ ±%5,2.

    Testin ilk hâli pazartesiye de düz %3 uyguluyordu. O tavanla hafta sonu
    boşluğu bir hafta içi gününden DAHA GÜVENLİ oluyordu: üç günlük haber
    tek günün bandına sıkışıyor, mekaniğin bütün amacı siliniyordu.

    Ölçüm (60 çekirdek × 140 gün): hafta içi sd %1,10, pazartesi sd %1,93 —
    yani 1,74×, √3'ün ta kendisi. En kötü pazartesi −%5,01, tavanın içinde.
  */
  const capFor = (day: number) => MARKET_DAILY_CAP * Math.sqrt(closedDaysBefore(day) + 1);

  it('her açık gün KENDİ güvenlik tavanını aşmaz', () => {
    for (let seed = 1; seed <= 100; seed += 1) {
      ...
      expect(capFor(8)).toBeCloseTo(MARKET_DAILY_CAP * Math.sqrt(3), 6);
  ...
  it('pazartesi boşluğu hafta içi bir günden BELİRGİN daha oynaktır', () => {
    const weekday: number[] = [];
    const monday: number[] = [];

    for (let seed = 1; seed <= 120; seed += 1) {
      const thursday = series(seed, 4);
      const friday = createMarketForDay(seed, 5, thursday);
      weekday.push(Math.abs(friday.goldSpot / thursday.goldSpot - 1));

      const saturday = createMarketForDay(seed, 6, friday);
      const sunday = createMarketForDay(seed, 7, saturday);
      const nextMonday = createMarketForDay(seed, 8, sunday);
      monday.push(Math.abs(nextMonday.goldSpot / friday.goldSpot - 1));
    }

    const avg = (a: number[]) => a.reduce((x, y) => x + y, 0) / a.length;
    const ratio = avg(monday) / avg(weekday);
    expect(ratio, `pazartesi/hafta içi ${ratio.toFixed(2)}×`).toBeGreaterThan(1.35);
