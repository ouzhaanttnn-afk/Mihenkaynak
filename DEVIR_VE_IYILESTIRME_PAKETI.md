...
Tarih: 2 Eylül 2026 · Dal: `claude/mobile-game-shop-system-8kyrmi` · Son commit: `2dafe6b`
...
- **Clone deposuna (`ouzhaanttnn-afk/clonemihenk`) dokunulmaz** — yalnız okunur.
- Test ortamı `node`; **`localStorage` yok**, tüm depolama try/catch içinde.
- Açıkça istenmedikçe **pull request açılmaz**. Push yalnız yukarıdaki dala.
...
Durum: **928 test / 55 dosya geçiyor**, `tsc --noEmit` temiz, üretim derlemesi temiz.
...
# clone (node_modules paylaşılabilir, package.json aynı)
cd /home/user/ouzhaanttnn-afk/clonemihenk
ln -sfn /home/user/Mihenkaynak/node_modules node_modules
...
### A. Bizim depo — hatalar

#### A1 · E · Bildirim balonları kapanmıyor, içeriği örtüyor
`src/ui/App.tsx:112-120`

Tek bir 4 saniyelik zamanlayıcı var ve `toasts` her değiştiğinde sıfırlanıyor. Arka arkaya
balon gelirse hiçbiri kapanmıyor; 7. günde ekranda hâlâ "Gün 5 kapandı" duruyordu ve balon,
dükkân kartının başlığını örtüyordu.

**Yapılacak:** zamanlayıcı balon başına çalışsın (her balon kendi `id`'siyle kendi
`setTimeout`'unu kursun), ve balon yığını dükkân kartının üstüne binmesin — üst şeridin
altına kaydırılsın.

#### A2 · E · İşletme alt rotasından çıkılamıyor
`src/ui/screens/BusinessScreen.tsx`

Piyasa / İşlem Defteri / Toptancı gibi bir alt rotaya girdikten sonra alttaki **İşletme**
sekmesine basmak hiçbir şey yapmıyor. Tek çıkış "← İşletme" bağlantısı. Sunumda kolayca
takılınacak yer. (Clone'da da aynı.)

**Yapılacak:** zaten aktif olan sekmeye ikinci dokunuş, o sekmenin kök rotasına dönsün.
...
### B. Bizim depo — tasarım ve oynanış
